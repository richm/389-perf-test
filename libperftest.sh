...
DATEFMT=${DATEFMT:-"%Y-%m-%d_%H:%M:%S%:z"}
DT=${DT:-`date +$DATEFMT`}
# this is the name of the remote log dir
LOGPREF=${LOGPREF:-log.}
LOGDIR=${LOGDIR:-"./${LOGPREF}$DT"}
INST=${INST:-perftest}
...
DB1=${DB1:-one}
DB2=${DB2:-two}
SUF1=${SUF1:-"ou=$DB1,$SUFFIX"}
SUF2=${SUF2:-"ou=$DB2,$SUFFIX"}
MODSUF1=${MODSUF1:-"ou=People,$SUF1"}
MODSUF2=${MODSUF2:-"ou=People,$SUF2"}
RAMDISK=${RAMDISK:-/dev/shm/slapd-$INST}
...
LDCLT=${LDCLT:-$USRPREFIX/bin/ldclt-bin}
S1NPROCS=${S1NPROCS:-4}
...
setupds() {
    if [ -f $DIR/setupperftest.inf ] ; then
        setupfilearg="-f $DIR/setupperftest.inf"
    fi
    setupargs="-l /dev/null -s $setupfilearg slapd.ServerPort=$PORT slapd.RootDNPwd=$ROOTPW slapd.Suffix=$SUFFIX General.FullMachineName=$HOST slapd.ServerIdentifier=$INST"
    ...
    $USRPREFIX/sbin/setup-ds.pl $setupargs
    ...
    ldapsearch -x -h localhost -p $PORT -D "$ROOTDN" -w "$ROOTPW" -b cn=config 'nsslapd-cachememsize=*' dn | \
        while read dnline ; do
            if [ -z "$dnline" ] ; then continue ; fi
...
dogenldif() {
    rm -f $DIR/$1.ldif $DIR/$1.ldif.bz2
    {
        echo dn: $2
        ...
        echo objectclass: organizationalUnit
        echo ""
        $USRPREFIX/bin/dbgen.pl -o - -n $NENTRIES -b 1 -j $NDIGITS -s "$2" -x -y
...
doloaddb() {
    if [ -z "$LDIF2DB" ] ; then
        if [ -f $USRPREFIX/sbin/ldif2db ] ; then
            cmd=$USRPREFIX/sbin/ldif2db
            extraargs="-Z $INST"
        elif [ -f $USRPREFIX/lib/dirsrv/slapd-$INST/ldif2db ] ; then
            cmd=$USRPREFIX/lib/dirsrv/slapd-$INST/ldif2db
        elif [ -f $USRPREFIX/lib64/dirsrv/slapd-$INST/ldif2db ] ; then
            cmd=$USRPREFIX/lib64/dirsrv/slapd-$INST/ldif2db
        fi
    else
        cmd="$LDIF2DB"
    fi
    if [ -f $DIR/$1.ldif ] ; then
        cat $DIR/$1.ldif | $cmd $extraargs -n $1 -i -
    else
        bzip2 -dc $DIR/$1.ldif.bz2 | $cmd $extraargs -n $1 -i -
    fi
}
...
stopds() {
    $USRPREFIX/sbin/stop-dirsrv $INST
}

startds() {
    $USRPREFIX/sbin/start-dirsrv $INST
}

restartds() {
    $USRPREFIX/sbin/restart-dirsrv $INST
}

cleanup() {
    $USRPREFIX/sbin/remove-ds.pl -i slapd-$INST
}
...
domod() {
    ...
    $LDCLT -h $HOST -p $PORT -D "$LDCLTDN" -w "$LDCLTPW" \
        $ASYNC \
        -e $opts,attreplace=sn:valueXXXXXXXXXX \
        $EXTRALDCLTOPTS \
        -r${STARTNUM:-1} -R${ENDNUM:-$NENTRIES} \
        -n$NTHREADS \
        -f $LDCLTFLT -b "$1" \
...
#        -e timestamp,sampinterval=$INTERVAL \
doadd() {
    opts=random
    if [ "$USEINCR" = 1 ] ; then
        opts="incr,commoncounter"
    else
        IGNOREALREADYEXISTS="-I 68"
    fi
    $LDCLT -h $HOST -p $PORT -D "$LDCLTDN" -w "$LDCLTPW" \
        -e add,person,$opts,noloop \
        -r${STARTNUM:-1} -R${ENDNUM:-$NENTRIES} \
        -n$NTHREADS $IGNOREALREADYEXISTS \
        -f ${LDCLTFLT:-$LDCLTFLT1} -b "$1" \
        -q &
    sleep $DURATION
    kill -2 %1
}
...
dodel() {
    ...
    $LDCLT -h $HOST -p $PORT -D "$LDCLTDN" -w "$LDCLTPW" \
        -e delete,$opts,noloop \
        -r${STARTNUM:-1} -R${ENDNUM:-$NENTRIES} \
        -n$NTHREADS $IGNOREMISSING \
        -f ${LDCLTFLT:-$LDCLTFLT1} -b "$1" \
        -q &
    sleep $DURATION
    kill -2 %1
...
# NOTE: -H cannot be saved - if you want threads, use top -H ...
...
cnvtpingstats() {
#1368560951
#PING ldap.example.com (10.16.65.79) 56(84) bytes of data.
#64 bytes from ldap.example.com (10.16.65.79): icmp_seq=1 ttl=64 time=1.48 ms
    awk -F '[ =]+' -v hroff=2 '
...
wait_for_server_up() {
    # systemd returns too soon - server pid file not written yet
    while true ; do
        DSPIDFILE=${DSPIDFILE:-/var/run/dirsrv/slapd-$INST.pid}
        DSPID=`cat $DSPIDFILE 2> /dev/null`
        realpid=`pidof ns-slapd 2> /dev/null`
        if [ "$realpid" = "$DSPID" ] ; then
            echo server is really running now
            break
        fi
        sleep 1
    done
...
wait_for_server_down() {
    # systemd returns too soon
    while true ; do
        realpid=`pidof ns-slapd 2> /dev/null`
        if [ -z "$realpid" ] ; then
            echo server is really stopped now
            break
        fi
        sleep 1
    done
...
myservice() {
    ${PREFIX:-/usr}/sbin/$2-$1 $3
    case "$2" in
        *start) wait_for_server_up ;;
        stop) wait_for_server_down ;;
    esac
}
